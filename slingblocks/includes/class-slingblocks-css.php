...
if ( ! class_exists( 'SLINGBLOCKS_CSS' ) ) {

	/**
	 * Class to create a minified css output.
	 */
	class SLINGBLOCKS_CSS {
			...
			if ( is_string( $value ) ) {
				...
				if ( isset( $value['top'] ) && isset( $value['bottom'] ) && isset( $value['left'] ) && isset( $value['right'] ) ) {
					...
					if ( ! empty( $value['top'] ) && $value['top'] === $value['right'] && $value['right'] === $value['left'] && $value['left'] === $value['bottom'] ) {
						...
						return $this;
					} else if ( ( $value['top'] || '0' === (string) $value['top'] ) && ( $value['right'] || '0' === (string) $value['right'] ) && ( $value['bottom'] || '0' === (string) $value['bottom'] ) && ( $value['left'] || '0' === (string) $value['left'] ) ) {
		...
		public function add_border_property( $property, $value ) {
			if ( is_array( $value ) && count( array_filter( $value ) ) > 0 ) {
				...
				if ( ( ! empty( $border_top ) || '0' === (string) $border_top ) && ( ! empty( $border_right ) || '0' === (string) $border_right ) && ( ! empty( $border_bottom ) || '0' === (string) $border_bottom ) && ( ! empty( $border_left ) || '0' === (string) $border_left ) ) {
